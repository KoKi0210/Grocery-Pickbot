...
public class RouteServiceImpl implements RouteService {
   ...
   *
   * @param order the order for which to calculate the path
   */
  ...
  private static List<int[]> calculateOptimalRoute(List<Product> products) {
    ...
    while (!productLocations.isEmpty()) {
      int[] nearest = findNearest(current,
          productLocations); // Намираме най-близката следваща лоакция на продукт
      route.addAll(generateRouteBetween(current,
          nearest)); // Добавяме пътя от сегашната локация до следващата най-близка
    ...
    route.addAll(generateRouteBetween(current,
        new int[]{0, 0})); // След като сме на последната локация се връщаме на [0, 0]
  ...
  private static List<int[]> generateRouteBetween(int[] start, int[] end) {
    ...
    while (!Arrays.equals(current, end)) {
      if (current[0] != end[0]) {
        current[0] += (end[0] > current[0]) ? 1 : -1; // Движение по X ос
      } else {
        current[1] += (end[1] > current[1]) ? 1 : -1; // Движение по Y ос
      }
  ...
  private static int[] findNearest(int[] current, List<int[]> locations) {
    ...
    for (int[] loc : locations) {
      double distance =
          Math.abs(loc[0] - current[0]) + Math.abs(loc[1] - current[1]); // изчислява разтоянието
